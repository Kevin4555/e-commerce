...
function ProductMini({ product }) {
  return (
    <div className="col-6 col-md-4 col-lg-3 mb-5 p-relative" id="product-mini">
      <div id="productContainer" className="shadow rounded">
        ...
        <Link to={`/product/${product.id}`} className="text-decoration-none">
          <div className="border px-2 rounded-bottom" id="infoContainer">
            <div className="row">
              <h5 className="mt-3">{product.title}</h5>
              <small className="fw-light col-12 mb-2" id="product">
                {product.description.length >= 80
                  ? product.description.substring(0, 80) + "..."
                  : product.description}
              </small>
              <span>
                <small id="priceBefore">{Math.floor(product.price * 1.2)} USD</small>
                <small> 20% OFF</small>
              </span>
